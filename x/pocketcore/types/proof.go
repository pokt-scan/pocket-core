...
type Proof interface {
	...
	Store(max sdk.BigInt, address sdk.Address)                                                           // handle the proof after validation
...
func (rp RelayProof) ValidateLocal(appSupportedBlockchains []string, sessionNodeCount int, sessionBlockHeight int64, verifyAddrs []sdk.Address) sdk.Error {
	//Basic Validations
...
func (rp RelayProof) Store(maxRelays sdk.BigInt, address sdk.Address) {
	// add the Proof to the global (in memory) collection of proofs
	SetProof(rp.SessionHeader(), RelayEvidence, rp, maxRelays, address)
}
...
func (c ChallengeProofInvalidData) Store(maxChallenges sdk.BigInt, address sdk.Address) {
	// add the Proof to the global (in memory) collection of proofs
	SetProof(c.SessionHeader(), ChallengeEvidence, c, maxChallenges, address)
}
