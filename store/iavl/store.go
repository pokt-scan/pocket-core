...
//}
//
//func (st *Store) SetPruning(options types.PruningOptions) {
//	panic("implement me")
//}
//
...
// LoadStore loads the iavl store
func LoadStoreSimple(db dbm.DB, id types.CommitID, lazyLoading bool) (*Store, error) {
	var err error
	...
	iavl := UnsafeNewStoreNoCache(tree, int64(0), int64(0))

	return iavl, nil
}

// LoadStore loads the iavl store
func LoadStore(db dbm.DB, id types.CommitID, pruning types.PruningOptions, lazyLoading bool) (*Store, error) {
	var err error
...
// CONTRACT: tree should be fully loaded / waves version.
// nolint: unparam
func UnsafeNewStoreNoCache(tree *MutableTree, numRecent int64, storeEvery int64) *Store {
	st := &Store{
		tree:       tree,
		numRecent:  numRecent,
		storeEvery: storeEvery,
